...
# def answer_set_element_to_string(
#     element: AnswerSetElement, *, flip: bool = False
# ) -> str:
...
#     atoms_in_the_answer_set = set(
#         element if type(element) is GroundAtom else element[0] for element in answer_set
#     )
...
#         SymbolicRule.parse(
#             "{"
#             + "; ".join(str(atom) for atom in atoms_in_the_answer_set)
#             + f"}} :- {false_predicate}."
#         ),
...
#     on_core.res = []
#     control.solve(
#         assumptions=[selector_to_literal[selector] for selector in selectors] + [-1],
#         on_core=on_core,
#     )
...
#         extra_atoms=known_atoms, compact=compact
#     )
...
#         *([rule for rule in extended_program][:-3])
#     )  # MALVI: DISCARD rules with __false__
...
#             atom = GroundAtom(at)
#             if (
#                 atom.predicate_name == "assign"
#             ):  # :- atom(Atom), #count{Value, Reason : assign(Atom, Value, Reason)} > 1.
#                 key = (atom.predicate_name, atom.arguments[0])
#             elif (
#                 atom.predicate_name == "cannot_support"
#             ):  # :- head(Rule, HeadAtom), #count{Atom : cannot_support(Rule, HeadAtom, Atom)} > 1.
#                 key = (atom.predicate_name, atom.arguments[0], atom.arguments[1])
#             elif (
#                 atom.predicate_name == "constraint"
#             ):  # :- rule(Rule), #count{Bound : constraint(Rule, Bound)} > 1.
#                 key = (atom.predicate_name, atom.arguments[0])
...
#     print_with_title(
#         "DERIVATION SEQUENCE",
#         res.filter(when=lambda atom: atom.predicate_name == "assign").as_facts,
#     )
#     return res
